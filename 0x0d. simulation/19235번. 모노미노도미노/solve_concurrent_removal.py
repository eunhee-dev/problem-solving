""" solve_concurrent_removal.py for 19235번. 모노미노도미노 """
...
def add_block(board: list[list[int]], t: int, row: int, col: int) -> None:
    ...
    while row + dr <= 5 and board[row][col] == 0 and board[row + dr][col + dc] == 0:
        row += 1
    row -= 1
    b_id = next(block_id)
...
def check_filled(board: list[list[int]]) -> tuple[int, int]:
    """ 완성된 행을 위에서부터 탐색하여 동시에 모두 제거 """
    curr_score = 0
    top_row = 6
    for r in range(5, 1, -1):
        if all(board[r][c] for c in range(4)):
            ...
            curr_score += 1
            top_row = r
            if curr_score == 2:  # 두 행 이상 동시에 제거될 수 없음 (최적화)
                break
    return curr_score, top_row
...
def play(board: list[list[int]], t: int, col: int) -> int:
    ...
    # 완성된 행 체크하여 제거 (완성된 행이 2개라면 동시에 제거)
    while True:
        curr_score, row = check_filled(board)
        if curr_score == 0:
            break
        score += curr_score
