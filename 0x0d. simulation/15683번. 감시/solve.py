...
def solve(n: int, m: int, board: list[list[str]]) -> int:
    ...
    def backtrack(i: int, _board: list[list[str]]) -> None:
        ...
        if i == len(cctv):
            ...
            return
        x, y, cctv_mode = cctv[i]
        for dir_comb in CCTV_MODES[cctv_mode]:
            modified = mark(_board, x, y, dir_comb)
            backtrack(i + 1, _board)
            unmark(_board, modified)
